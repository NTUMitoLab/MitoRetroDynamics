...
|Folder|Description|
|---|---|
|[Analysis on multiple solutions](FrequencyResponse/notebooks)|These notebooks are generated by other solutions. The primary solution is used in [FrequencyResponse.ipynb](FrequencyResponse.ipynb). Noted that the save error shown in each notebook is expected because these notebooks are directly converted by [run_sol.jl](run_sol.jl)|
|[Effects of RTG concentrations](PlottingMakie/result/)|Concentrations of RTG proteins are manipulated to investigate the effect of expression level on signaling|
